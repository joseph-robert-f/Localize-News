/**
 * Scraper: City of Atlanta, GA
 *
 * Target: https://atlantaga.legistar.com/Calendar.aspx
 * Documents: City Council agendas and meeting minutes via Legistar
 *
 * Run in isolation:
 *   npx tsx scrapers/atlanta-ga.ts
 */

import { scrapeLegistar } from "./legistar";
import type { ScraperResult } from "./types";

const TOWNSHIP_ID_PLACEHOLDER = "00000000-0000-0000-0000-000000000022";

export async function scrapeAtlantaGA(
  townshipId = TOWNSHIP_ID_PLACEHOLDER
): Promise<ScraperResult> {
  console.log("[atlanta-ga] Scraping via Legistar…");
  const { documents, errors } = await scrapeLegistar("atlantaga");
