/**
 * Scraper: City of Boston, MA
 *
 * Target: https://boston.legistar.com
 * Documents: City Council agendas and meeting minutes via Legistar
 *
 * Run in isolation:
 *   npx tsx scrapers/boston-ma.ts
 */

import { scrapeLegistar } from "./legistar";
import type { ScraperResult } from "./types";

const TOWNSHIP_ID_PLACEHOLDER = "00000000-0000-0000-0000-000000000010";

export async function scrapeBostonMA(
  townshipId = TOWNSHIP_ID_PLACEHOLDER
): Promise<ScraperResult> {
  console.log("[boston-ma] Scraping via Legistar…");
  const { documents, errors } = await scrapeLegistar("boston");
