/**
 * Scraper: City of Providence, RI
 *
 * Target: https://providenceri.legistar.com
 * Documents: City Council agendas and meeting minutes via Legistar
 *
 * Run in isolation:
 *   npx tsx scrapers/providence-ri.ts
 */
...
export async function scrapeProvidenceRI(
  townshipId = TOWNSHIP_ID_PLACEHOLDER
): Promise<ScraperResult> {
  console.log("[providence-ri] Scraping via Legistar…");
  const { documents, errors } = await scrapeLegistar("providenceri");
