/**
 * Scraper: City of Raleigh, NC
 *
 * Target: https://raleigh.legistar.com
 * Documents: City Council agendas and meeting minutes via Legistar
 *
 * Run in isolation:
 *   npx tsx scrapers/raleigh-nc.ts
 */

import { scrapeLegistar } from "./legistar";
import type { ScraperResult } from "./types";

const TOWNSHIP_ID_PLACEHOLDER = "00000000-0000-0000-0000-000000000018";

export async function scrapeRaleighNC(
  townshipId = TOWNSHIP_ID_PLACEHOLDER
): Promise<ScraperResult> {
  console.log("[raleigh-nc] Scraping via Legistar…");
  const { documents, errors } = await scrapeLegistar("raleigh");
