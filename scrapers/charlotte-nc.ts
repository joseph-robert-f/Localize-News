/**
 * Scraper: City of Charlotte, NC
 *
 * Target: https://charlotte.legistar.com
 * Documents: City Council agendas and meeting minutes via Legistar
 *
 * Run in isolation:
 *   npx tsx scrapers/charlotte-nc.ts
 */

import { scrapeLegistar } from "./legistar";
import type { ScraperResult } from "./types";

const TOWNSHIP_ID_PLACEHOLDER = "00000000-0000-0000-0000-000000000017";

export async function scrapeCharlotteNC(
  townshipId = TOWNSHIP_ID_PLACEHOLDER
): Promise<ScraperResult> {
  console.log("[charlotte-nc] Scraping via Legistar…");
  const { documents, errors } = await scrapeLegistar("charlotte");
