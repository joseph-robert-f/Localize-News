/**
 * Scraper: City of Buffalo, NY
 *
 * Target: https://buffalo.legistar.com
 * Documents: City Council agendas and meeting minutes via Legistar
 *
 * Run in isolation:
 *   npx tsx scrapers/buffalo-ny.ts
 */

import { scrapeLegistar } from "./legistar";
import type { ScraperResult } from "./types";

const TOWNSHIP_ID_PLACEHOLDER = "00000000-0000-0000-0000-000000000019";

export async function scrapeBuffaloNY(
  townshipId = TOWNSHIP_ID_PLACEHOLDER
): Promise<ScraperResult> {
  console.log("[buffalo-ny] Scraping via Legistar…");
  const { documents, errors } = await scrapeLegistar("buffalo");
